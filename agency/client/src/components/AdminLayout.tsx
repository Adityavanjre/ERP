...
const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-zinc-100 flex font-sans selection:bg-[#C5A059]/30">
      ...
      >
        ...
        <div className="h-24 flex items-center px-8 border-b border-white/5">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-[#C5A059] rounded-xl flex items-center justify-center font-black text-black shadow-lg shadow-[#C5A059]/10 rotate-3">
              K
            </div>
            {sidebarOpen && (
              <div className="flex flex-col">
                <span className="font-bold tracking-tight text-lg text-white font-heading">
                  KLYPSO
                </span>
                <span className="text-[#C5A059] font-black text-[9px] uppercase tracking-[0.4em] leading-none mt-1">
                  Console v.02
                </span>
              </div>
            )}
            ...
            <div key={gIdx} className="space-y-3">
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`${isActive ? "text-[#C5A059]" : "text-zinc-600 group-hover:text-zinc-400"} transition-colors shrink-0`}
                    >
                      {item.icon}
                    </div>
                    {sidebarOpen && (
                      <span
                        className={`font-bold text-sm tracking-tight ${isActive ? "text-white" : ""}`}
                      >
                        {item.label}
                      </span>
                    )}
                    {isActive && sidebarOpen && (
                      <motion.div
                        layoutId="nav-glow"
                        className="ml-auto w-1 h-1 rounded-full bg-[#C5A059] gold-glow"
                      />
                    )}
                  </Link>
                );
        ...
        <div className="p-6 border-t border-white/5 bg-[#0A0A0B]/50 relative z-10">
          <div
            className={`p-4 rounded-3xl bg-white/[0.02] border border-white/5 flex items-center gap-4 ${sidebarOpen ? "" : "justify-center"}`}
          >
            ...
            {sidebarOpen && (
              <button
                ...
                className="text-zinc-600 hover:text-[#C5A059] p-2 transition-colors"
              >
                <LogOut size={16} />
              </button>
            )}
      ...
      {/* Main Content Area */}
      <main
        className={`flex-1 flex flex-col transition-all duration-500 ease-[0.2,1,0.3,1] ${sidebarOpen ? "lg:ml-72" : "lg:ml-24"} ml-0`}
      >
        {/* Header */}
        <header className="h-20 lg:h-24 bg-[#0A0A0B]/60 backdrop-blur-xl sticky top-0 z-40 px-4 lg:px-10 flex items-center justify-between border-b border-white/5">
          ...
          <div className="flex items-center gap-6">
            ...
            <a
              href="/"
              target="_blank"
              className="bg-[#C5A059] hover:bg-[#D4AF37] text-black h-10 lg:h-12 px-4 lg:px-6 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-3 shadow-lg shadow-[#C5A059]/10"
            >
