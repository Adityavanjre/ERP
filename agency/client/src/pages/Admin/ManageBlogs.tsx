...
const ManageBlogs = () => {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      setDeleteError(
        errorObj.response?.data?.message || "Failed to delete article",
      );
  ...
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center py-24 text-zinc-500 gap-4">
        ...
        <p className="text-sm font-bold uppercase tracking-widest text-[#C5A059]/50 font-heading">
          Curating Boutique Insights...
        </p>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm font-medium">
            Manage and publish your boutique insights and resources.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <Link
            to="/admin/add-blog"
            className="bg-[#C5A059] hover:bg-[#D4AF37] text-black px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 shadow-lg shadow-[#C5A059]/10"
          >
        ...
        <AnimatePresence>
          {filteredBlogs.length > 0 ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-4 mb-3 text-[9px] font-black text-zinc-500 uppercase tracking-widest">
                    ...
                    <div className="flex items-center gap-1.5">
                      <User size={12} className="text-zinc-600" /> BY{" "}
                      {blog.author || "TEAM KLYPSO"}
                    </div>
                    <div
                      className={`px-2 py-0.5 rounded border ${blog.status === "published" ? "border-emerald-500/20 text-emerald-500 bg-emerald-500/5" : "border-amber-500/20 text-amber-500 bg-amber-500/5"}`}
                    >
                      {blog.status || "draft"}
                    </div>
                  </div>
                  ...
                  <p className="text-zinc-500 text-sm font-medium line-clamp-2 max-w-3xl leading-relaxed">
                    {blog.excerpt}
                  </p>
                  ...
                  {blog.tags && blog.tags.length > 0 && (
                    <div className="flex gap-2 mt-4">
                      {blog.tags.map((tag: string, i: number) => (
                        <span
                          key={i}
                          className="text-[7px] font-black uppercase tracking-[0.2em] text-[#C5A059]/40 py-1 px-2 border border-[#C5A059]/10 rounded-md bg-[#C5A059]/5"
                        >
                          #{tag}
                        </span>
                      ))}
            ...
            <div className="text-center py-32 bg-white/[0.01] rounded-3xl border border-dashed border-white/5">
              ...
              <p className="text-zinc-600 font-black uppercase tracking-widest text-[10px] mb-8">
                No articles published yet
              </p>
              <Link
                to="/admin/add-blog"
                className="text-[#C5A059] hover:text-white font-black uppercase tracking-widest text-[10px] underline transition-colors"
              >
                Create First Entry
              </Link>
            </div>
