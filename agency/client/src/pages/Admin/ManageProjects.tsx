    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm font-medium">
            Manage and deploy portfolio assets to the front-end.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <Link
            ...
            className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl font-bold text-xs transition-all flex items-center gap-2 shadow-lg shadow-indigo-600/10"
          >
            <Plus size={16} />
            Deploy New
          </Link>
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-sm">
                    {project.link && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="p-3 bg-white text-black rounded-xl hover:bg-zinc-200 transition-all shadow-xl"
                        title="View Live"
                      >
                    ...
                    <Link
                      ...
                      className="p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 transition-all shadow-xl"
                      title="Edit Content"
                    >
                      <Edit size={16} />
                    </Link>
                    <button
                      ...
                      className="p-3 bg-red-600 text-white rounded-xl hover:bg-red-500 transition-all shadow-xl"
                      title="Delete Asset"
                    >
                      <Trash2 size={16} />
                    </button>
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {project.categories.slice(0, 2).map((cat, i) => (
                      <span
                        key={i}
                        className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded bg-black text-zinc-400 border border-white/5"
                      >
                        {cat}
                      </span>
                    ))}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-black/20">
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500 border-b border-white/5">
                  Asset
                </th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500 border-b border-white/5">
                  Categories
                </th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500 border-b border-white/5 text-right">
                  Actions
                </th>
              </tr>
            </thead>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 font-medium truncate max-w-xs">
                          {project.description}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-1 flex-wrap">
                      {project.categories.map((cat, i) => (
                        <span
                          key={i}
                          className="text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-black border border-white/5 text-zinc-500"
                        >
                          {cat}
                        </span>
                      ))}
      ...
      {filteredProjects.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 bg-white/[0.02] rounded-3xl border border-dashed border-white/5 text-center">
          ...
          <Link
            to="/admin/add-project"
            className="text-indigo-400 hover:text-white font-bold underline transition-colors"
          >
