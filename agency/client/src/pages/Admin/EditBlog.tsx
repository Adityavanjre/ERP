  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        setError(
          errorObj.response?.data?.message || "Failed to fetch article details",
        );
      } finally {
        setFetching(false);
      }
  ...
  if (fetching)
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-zinc-500 gap-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-[#C5A059]/50">
          Reconstructing Workstation...
        </p>
      </div>
    );
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div>
            ...
            <h1 className="text-4xl font-bold font-heading tracking-tight flex items-center gap-4 text-white">
              Modify Insight
              <span
                className={`text-[10px] uppercase tracking-widest px-3 py-1 rounded-full border ${status === "published" ? "border-emerald-500/30 text-emerald-500 bg-emerald-500/5" : "border-amber-500/30 text-amber-500 bg-amber-500/5"}`}
              >
                {status}
              </span>
            </h1>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Save size={16} className="text-[#C5A059]" /> Save changes as{" "}
              {status === "published" ? "Draft" : "Published"}
            </button>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="space-y-8">
            <div className="bg-[#141417] border border-white/5 rounded-[2rem] p-8 space-y-8 shadow-xl">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C5A059] border-b border-white/5 pb-4">
                Metadata Engine
              </h3>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <LinkIcon
                      className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-700 group-focus-within:text-[#C5A059] transition-colors"
                      size={12}
                    />
                    <input
                      type="text"
