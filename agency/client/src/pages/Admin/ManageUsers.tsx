...
const ManageUsers = () => {
  ...
  const handleToggleRole = async (id: string, currentIsAdmin: boolean) => {
    if (
      !window.confirm(
        `Are you sure you want to ${currentIsAdmin ? "demote" : "promote"} this user?`,
      )
    )
      return;
    try {
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u._id === id ? { ...u, isAdmin: !currentIsAdmin } : u,
        ),
      );
    } catch (error: unknown) {
      const err = error as { response?: { data?: { message?: string } } };
  ...
  const handleDeleteUser = async (id: string) => {
    if (
      !window.confirm(
        "Erase this user from the system? This action cannot be undone.",
      )
    )
      return;
    try {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    try {
      setIsResetting(true);
      await api.put(`/api/users/${selectedUser._id}/password`, {
        ...
        <div className="w-1/3 min-w-[320px] flex flex-col bg-[#141417] border border-white/5 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-5 border-b border-white/5 space-y-4 bg-black/20">
            <div className="flex items-center justify-between">
              ...
              <span className="bg-white/5 px-2 py-0.5 rounded-md text-[10px] font-bold text-zinc-400 font-mono">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
            {filteredUsers.length === 0 ? (
              <div className="text-center py-20 text-zinc-600 font-medium text-xs">
              ...
              filteredUsers.map((u) => (
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <h3
                      className={`font-bold text-sm tracking-tight ${selectedId === u._id ? "text-white" : "text-zinc-300 group-hover:text-white"}`}
                    >
                      {u.name}
                    </h3>
                    <div className="flex items-center gap-1">
                      ...
                      <span
                        className={`text-[9px] font-bold uppercase tracking-widest ${selectedId === u._id ? "text-[#C5A059]/60" : "text-zinc-600"}`}
                      >
                  ...
                  <div className="flex justify-between items-center">
                    <p
                      className={`text-[10px] font-bold tracking-tight opacity-60 italic ${selectedId === u._id ? "text-[#C5A059]/80" : "text-zinc-500"}`}
                    >
                      {u.email}
                    </p>
                  ...
                  {selectedId === u._id && (
                    <motion.div
                      ...
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-[#C5A059] rounded-r-full"
                    />
                  )}
                </button>
              ))
                ...
                <div className="p-8 border-b border-white/5 bg-black/10 flex justify-between items-start">
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C5A059] px-3 py-1 bg-[#C5A059]/10 rounded-full border border-[#C5A059]/10">
                        System Identity
                      </span>
                    ...
                    <h2 className="text-4xl font-bold text-white tracking-tight font-heading">
                      {selectedUser.name}
                    </h2>
                  ...
                  <div
                    className={`p-4 rounded-2xl border ${selectedUser.isAdmin ? "bg-[#C5A059]/10 border-[#C5A059]/20 text-[#C5A059]" : "bg-zinc-900 border-white/5 text-zinc-500"}`}
                  >
                ...
                <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <InfoBlock
                      ...
                      isLink
                      link={`mailto:${selectedUser.email}`}
                    />
                  </div>
                  ...
                  <div className="space-y-6">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-600 flex items-center gap-3">
                      <Lock size={14} className="text-[#C5A059]" /> Access
                      Configuration
                    </h3>
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide">
                            Click to toggle privilege
                          </p>
                        </div>
                        <div
                          className={`w-12 h-6 rounded-full relative transition-colors ${selectedUser.isAdmin ? "bg-[#C5A059]" : "bg-zinc-800"}`}
                        >
                          <div
                            className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${selectedUser.isAdmin ? "left-7" : "left-1"}`}
                          />
                        </div>
                      </button>
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 space-y-4">
                      ...
                      <p className="text-[10px] text-zinc-500 font-medium tracking-wide">
                        Assigning a new key will immediately invalidate the
                        current credential tuple.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-8 border border-red-500/10 bg-red-500/5 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-[11px] font-medium text-zinc-400 leading-relaxed uppercase tracking-wider max-w-md">
                          Erasing an identity will permanently revoke their
                          access from the system. Use caution.
                        </p>
                      </div>
                    </div>
                    <button
...
const InfoBlock = ({ icon, label, value, isLink, link }: InfoBlockProps) => (
  <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 hover:border-[#C5A059]/20 transition-all group">
    <div className="flex items-center gap-3 mb-4">
      <span className="text-zinc-600 group-hover:text-[#C5A059] transition-colors">
        {icon}
      </span>
    ...
    {isLink ? (
      <a
        href={link}
        className="text-lg font-bold text-white hover:text-[#C5A059] transition-colors truncate block"
      >
        {value}
      </a>
    ) : (
