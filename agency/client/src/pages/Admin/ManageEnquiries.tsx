          ...
          <div className="p-5 border-b border-white/5 space-y-4 bg-black/20">
            <div className="flex items-center justify-between">
              <h2 className="font-bold flex items-center gap-2">
                <Inbox size={18} className="text-indigo-400" /> Inbox
              </h2>
              <span className="bg-white/5 px-2 py-0.5 rounded-md text-[10px] font-bold text-zinc-400">
                {filteredEnquiries.length} Logs
              </span>
            </div>
              ...
              filteredEnquiries.map((enq) => (
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <h3
                      className={`font-bold text-sm tracking-tight ${selectedId === enq._id ? "text-white" : "text-zinc-300 group-hover:text-white"}`}
                    >
                      {enq.name}
                    </h3>
                    <span
                      className={`text-[9px] font-bold uppercase tracking-widest ${selectedId === enq._id ? "text-indigo-400" : "text-zinc-600"}`}
                    >
                  ...
                  <div className="flex justify-between items-center">
                    <p
                      className={`text-[10px] font-bold uppercase tracking-widest ${selectedId === enq._id ? "text-indigo-300/60" : "text-zinc-500"}`}
                    >
                      {enq.service}
                    </p>
                ...
                <div className="p-8 border-b border-white/5 bg-black/10 flex justify-between items-start">
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 px-3 py-1 bg-indigo-500/10 rounded-full border border-indigo-500/10">
                        Inbound Intel
                      </span>
                    ...
                    <p className="text-zinc-500 text-xs font-medium flex items-center gap-2">
                      {selectedEnquiry.projectType || "General Request"} •
                      Received{" "}
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <InfoBlock
                      ...
                      isLink
                      link={`mailto:${selectedEnquiry.email}`}
                    />
                    <InfoBlock
                      icon={<DollarSign size={16} />}
                      label="Budget Allocation"
                      value={selectedEnquiry.budget || "Unspecified"}
                    />
                  </div>
                  ...
                  {selectedEnquiry.referenceLinks &&
                    selectedEnquiry.referenceLinks.length > 0 &&
                    selectedEnquiry.referenceLinks[0] !== "" && (
                      <div className="space-y-4">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-600 flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-white/5">
                    <InfoBlock
                      ...
                      value={selectedEnquiry.timeline || "On Demand"}
                    />
                    <InfoBlock
                      icon={<Briefcase size={16} />}
                      label="Project Classification"
                      value={selectedEnquiry.projectType || "Standard"}
                    />
                  </div>
                </div>
              </motion.div>
