...
const BlogDetails = () => {
  ...
  if (error || !post)
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#0A0A0B] text-white">
        <h1 className="text-4xl font-bold mb-4 font-heading">
          Article Not Found
        </h1>
        <Link
          to="/resources"
          className="text-[#C5A059] font-black uppercase tracking-[0.2em] text-[10px] hover:underline"
        >
          Back to Insights
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-white pt-48 pb-32 relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-5xl px-6 relative z-10">
        <Link
          to="/resources"
          className="inline-flex items-center gap-4 text-zinc-500 hover:text-white mb-16 transition-all group px-8 py-4 rounded-full bg-white/[0.02] border border-white/5 backdrop-blur-3xl"
        >
        ...
        >
          ...
          <div className="flex items-center justify-between mb-20 py-10 border-y border-white/5">
            <div className="flex items-center gap-8">
              ...
              <div>
                <p className="font-bold text-white text-lg font-heading">
                  {post.author || "Klypso Analyst"}
                </p>
                <p className="text-[#C5A059] text-[9px] font-black uppercase tracking-[0.4em]">
                  Managing Partner
                </p>
              </div>
            </div>
            <button
              onClick={sharePost}
              className="w-14 h-14 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center justify-center text-zinc-500 hover:text-[#C5A059] hover:border-[#C5A059]/30 transition-all group"
            >
              <Share2
                size={24}
          ...
          <div className="mt-32 p-12 bg-[#121214] border border-white/5 rounded-[3rem] flex flex-col md:flex-row items-center gap-12 group hover:border-[#C5A059]/30 transition-all duration-700">
            ...
            <div className="flex-1 text-center md:text-left">
              <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#C5A059] mb-4">
                About the Author
              </h4>
              <h3 className="text-2xl font-bold mb-4 font-heading">
                {post.author || "Klypso Team"}
              </h3>
              <p className="text-zinc-500 leading-relaxed max-w-xl font-medium">
                Architecting the future of digital experiences at Klypso.
                Focused on the intersection of technical excellence, aesthetic
                psychology, and high-performance engineering.
              </p>
            </div>
          </div>
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="mt-20 flex flex-wrap gap-4 pt-10 border-t border-white/5">
              {post.tags.map((tag, i) => (
                <span
                  key={i}
                  className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C5A059]/40 py-2 px-4 border border-[#C5A059]/10 rounded-full bg-[#C5A059]/5"
                >
                  #{tag}
                </span>
              ))}
          ...
          {relatedPosts.length > 0 && (
            <div className="mt-40">
              <div className="flex items-center gap-8 mb-16">
                <h2 className="text-4xl font-bold font-heading shrink-0 tracking-tight">
                  Expand Your Knowledge.
                </h2>
                <div className="h-[1px] flex-1 bg-white/5" />
                <Link
                  to="/resources"
                  className="text-[10px] font-black uppercase tracking-widest text-[#C5A059] hover:text-white transition-colors underline underline-offset-8"
                >
                  Explore All
                </Link>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {relatedPosts.map((rPost) => (
                  ...
                  >
                    ...
                    <h4 className="text-[9px] font-black text-[#C5A059] uppercase tracking-widest mb-4">
                      {rPost.category}
                    </h4>
                    <h3 className="font-bold text-lg mb-4 leading-snug font-heading group-hover:text-[#C5A059] transition-colors">
                      {rPost.title}
                    </h3>
                    <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-widest text-zinc-600">
                      ...
                      <ArrowLeft
                        size={12}
                        className="rotate-180 text-[#C5A059]"
                      />
                    </div>
                  </Link>
                ))}
          ...
          <div className="mt-40 pt-20 border-t border-white/5 text-center px-8 md:px-20 py-20 bg-[#121214] rounded-[4rem] border border-white/5 relative overflow-hidden">
            ...
            <h3 className="text-4xl md:text-6xl font-bold mb-8 font-heading tracking-tight">
              Stay Informed.
            </h3>
            <p className="text-zinc-500 mb-12 text-lg font-medium max-w-xl mx-auto">
              Join our newsletter to receive the latest updates on technology,
