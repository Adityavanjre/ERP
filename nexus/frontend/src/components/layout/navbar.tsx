...
export const Navbar = () => {
  ...
  return (
    <div className="flex items-center p-2.5 md:p-4 border-b border-slate-200 bg-white/80 backdrop-blur-xl sticky top-0 z-[60]">
      <MobileSidebar />
      <div className="flex-1 flex items-center">
        <div
          className="hidden md:flex items-center gap-2.5 px-4 py-2 rounded-2xl bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 hover:border-slate-300 transition-all cursor-pointer group shadow-sm"
          onClick={() =>
        ...
        >
          ...
          <div className="flex items-center gap-1.5 ml-6 border-l border-slate-300 pl-4">
            ...
            <span className="text-[10px] font-black tracking-tighter text-slate-600">
              K
            </span>
          </div>
        </div>
          ...
          <div className="hidden lg:flex items-center gap-1.5 p-1 bg-slate-50 border border-slate-200 rounded-2xl shadow-sm">
            ...
            <div
              onClick={async () => {
                if (
                  !confirm(
                    "SECURE CLOUD MERGE: This will bridge your local and cloud data using conflict resolution (like WhatsApp/Google Photos). Recent offline changes will be preserved. Proceed?",
                  )
                )
                  return;
                const bridge = (
                ...
                if (bridge?.sync?.bootstrap) {
                  try {
                    ...
                    if (result.error) {
                      ...
                      alert(
                        `Cloud Merge Successful! Synced ${result.pulledCount} critical records.`,
                      );
