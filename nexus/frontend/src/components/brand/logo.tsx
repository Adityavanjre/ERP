...
export const KlypsoLogo = ({ collapsed = false, size = 40, name = "KLYPSO" }: { collapsed?: boolean; size?: number; name?: string }) => {
    return (
        <div className="group flex items-center gap-4 select-none">
            ...
            >
                <span style={{ fontSize: size * 0.5 }}>{name.charAt(0).toUpperCase()}</span>
            </div>
            {!collapsed && (
                <span
