...
export function BOMDetailsDialog({
  bomId,
  open,
  onOpenChange,
}: BOMDetailsDialogProps) {
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-white hover:bg-white">
                    ...
                    <TableHead className="text-[10px] uppercase font-bold tracking-widest text-slate-400 text-right">
                      Qty / Batch
                    </TableHead>
                    <TableHead className="text-[10px] uppercase font-bold tracking-widest text-slate-400 text-right">
                      Unit Cost
                  ...
                  {bom.items?.map((item: BOMItemDetailed) => (
                    <TableRow key={item.id} className="hover:bg-slate-50">
                      ...
                      <TableCell className="text-right font-bold text-slate-900">
                        ₹
                        {(
                          (item.product?.costPrice || 0) * item.quantity
                        ).toLocaleString()}
                      </TableCell>
                    </TableRow>
                  ))}
