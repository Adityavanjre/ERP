...
export function ApiKeyManager() {
  ...
  const revokeKey = async (id: string) => {
    if (
      !confirm(
        "Are you sure? This will immediately break any external integrations using this key.",
      )
    )
      return;
    try {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="font-medium">
                Generate secure credentials for external integrations and
                automations.
              </CardDescription>
            </div>
              ...
              >
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-16 w-16 rounded-xl bg-white/20 hover:bg-white/30 text-white"
                  >
                    {copied ? (
                    ...
                    <div className="flex gap-2">
                      {key.scopes.map((s) => (
                        <Badge
                          key={s}
                          variant="secondary"
                          className="bg-blue-50 text-blue-600 border-none font-bold text-[9px] uppercase tracking-widest"
                        >
                          {s}
                        </Badge>
                      ))}
