...
export function AuditorDashboard() {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          const endpoint =
            action === "LOCK"
              ? "accounting/auditor/lock"
              : "accounting/auditor/unlock";
          await api.post(endpoint, {
            month,
            year,
            reason:
              action === "UNLOCK" ? "Administrative Reopening" : undefined,
          });
          ...
          toast.error(
            error.response?.data?.message ||
              error.message ||
              `Failed to ${action.toLowerCase()} period`,
          );
        } finally {
          setUILocked(false);
        }
  ...
  if (loading)
    return (
      <div className="p-2.5 text-center font-bold text-slate-400">
        Auditing books...
      </div>
    );
  ...
  return (
    <div className="space-y-1.5">
      {/* Date Selectors */}
      <div className="flex flex-col md:flex-row gap-2 items-end mb-2 bg-white p-3 rounded-3xl border border-slate-100 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        ...
        <Card className="lg:col-span-2 bg-white border-slate-200 shadow-xl shadow-slate-200/40 rounded-3xl overflow-hidden border-none">
          <CardHeader
            className={`${data?.isLocked ? "bg-slate-900 text-white" : "bg-amber-600 text-white"} p-2.5 transition-colors`}
          >
            <div className="flex justify-between items-start">
              ...
              <Button
                onClick={toggleLock}
                className={`${data?.isLocked ? "bg-white/10 hover:bg-white/20" : "bg-white text-amber-600 hover:bg-slate-50"} rounded-2xl font-black px-3`}
              >
          ...
          <CardContent className="p-2.5 space-y-1.5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                {
                  label: "Tally Readiness",
                  val: `${data?.confidenceScore}%`,
                  color: "text-emerald-500",
                  icon: ShieldCheck,
                },
                {
                  label: "HSN Coverage",
                  val: `${data?.hsnCoverage}%`,
                  color: "text-blue-500",
                  icon: FileSearch,
                },
                {
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                <AlertTriangle className="h-4 w-4 text-amber-500" /> Validation
                Messages
              </h3>
              <div className="space-y-1.5">
                {data?.errors?.map((err, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2 p-2.5 rounded-2xl bg-rose-50 border border-rose-100 text-rose-700 text-sm font-bold"
                  >
                ...
                {data?.errors?.length === 0 && (
                  <div className="flex items-start gap-2 p-3 rounded-3xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-sm font-bold">
                    <CheckCircle2 className="h-5 w-5 mt-0.5" />
                    All vouchers are balanced and ready for Tally export. No
                    drift detected.
                  </div>
                )}
              </div>
        ...
        <Card className="bg-white border-slate-200 shadow-xl shadow-slate-200/40 rounded-3xl overflow-hidden border-none">
          <CardHeader className="p-2.5 border-b border-slate-100">
            <CardTitle className="text-xl font-black">
              Risk Parameters
            </CardTitle>
            <CardDescription className="font-bold">
              Automated audit checkpoints
            </CardDescription>
          </CardHeader>
          <CardContent className="p-2.5 space-y-1.5">
            {data?.riskFlags?.map((flag, i) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div
                    className={`h-8 w-8 rounded-lg flex items-center justify-center ${flag.severity === "BLOCKER" ? "bg-rose-50 text-rose-500" : "bg-amber-50 text-amber-500"}`}
                  >
                ...
                <Badge
                  variant="outline"
                  className={`${flag.severity === "BLOCKER" ? "border-rose-200 text-rose-500 bg-rose-50" : "border-amber-200 text-amber-500 bg-amber-50"} font-black text-[10px]`}
                >
            ...
            {!data?.riskFlags?.length && (
              <div className="text-center py-2">
                <ShieldCheck className="mx-auto h-12 w-12 text-emerald-100 mb-2" />
            ...
            <div className="pt-2 border-t border-slate-100">
              <Button
                variant="ghost"
                className="w-full justify-between hover:bg-slate-50 rounded-2xl p-2.5 font-bold text-slate-600 transition-all group"
              >
                Download Audit Log
