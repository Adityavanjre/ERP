      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-0">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Monitor and manage your production orders and materials.
          </p>
        </div>
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start relative z-10 gap-4 sm:gap-0">
                  <div className="flex gap-4 sm:gap-8 items-start sm:items-center">
                    ...
                    <div>
                      <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] block mb-1">
                        WO-{wo.orderNumber}
                      </span>
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2">
                        <Badge
                          variant="secondary"
                          className="bg-slate-100 text-slate-500 font-black text-[9px] rounded-lg border-none uppercase text-ellipsis overflow-hidden whitespace-nowrap max-w-[150px]"
                        >
                          {wo.bom?.name || "Standard Unit"}
                        </Badge>
                        {wo.status !== "Completed" && (
                ...
                <div className="mt-8 sm:mt-10 pt-6 sm:pt-8 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center relative z-10 gap-4 sm:gap-0">
                  <div className="flex gap-3 sm:gap-5 items-center pl-[4rem] sm:pl-0">
                    <div className="flex -space-x-3">
                      {Array.from({
                        length: Math.min(wo.bom?.items?.length || 0, 3),
                      }).map((_, i) => (
                        <div
                          key={i}
                          className="w-8 h-8 rounded-xl bg-slate-100 border-2 border-white shadow-sm flex items-center justify-center text-[10px] font-black text-slate-400"
                        >
                          RC
                        </div>
                      ))}
                    ...
                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">
                      {wo.bom?.items?.length || 0} Materials Used
                    </span>
                  </div>
              ...
              <div className="space-y-8 relative z-10">
                <div>
                  ...
                  <p className="text-4xl font-black text-slate-900 tracking-tighter">
                    ...
                    <span className="text-sm font-bold ml-2 text-slate-400">
                      ORDERS
                    </span>
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-4xl font-black text-emerald-600 tracking-tighter">
                    {workOrders
                      .filter((w: WorkOrder) => w.status === "Completed")
                  ...
                  <div className="flex gap-5 items-center">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest mt-0.5">
                        {bom.items?.length || 0} Components Required
                      </p>
                    </div>
                  </div>
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-2">
              {activity.map((a, i) => (
                <div
                  key={i}
                  className="flex gap-3 items-start p-2 hover:bg-slate-50 rounded-xl transition-all"
                >
