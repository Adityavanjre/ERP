...
export default function SalesPage() {
  ...
  const syncSalesData = useCallback(async (showLoading = false) => {
    try {
      ...
      setCustomers(
        Array.isArray(custRes.data) ? custRes.data : custRes.data.data || [],
      );
    } catch {
    ...
    switch (status) {
      ...
      default:
        return (
          <Badge
            variant="secondary"
            className="bg-slate-100 text-slate-500 font-black text-[10px] uppercase tracking-tighter rounded-full px-3 py-1"
          >
            {status}
          </Badge>
        );
    ...
    <div className="flex-1 space-y-6 md:space-y-10 pt-2 md:pt-8 px-4 md:px-8 bg-slate-50/30 w-full max-w-full overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-0">
        <div>
          ...
          <p className="text-slate-600 mt-3 md:mt-2 font-black uppercase text-[10px] tracking-[0.2em] ml-0 md:ml-[68px]">
        ...
        <CardHeader className="bg-slate-50/50 border-b border-slate-100 py-10 px-10">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-0">
            <div>
              ...
              <CardDescription className="text-slate-600 font-black uppercase text-[10px] tracking-[0.2em] mt-2">
                All customer orders and invoices
              </CardDescription>
            </div>
            <Badge
              variant="outline"
              className="w-fit border-slate-200 bg-white text-slate-500 font-black px-4 py-1.5 rounded-xl uppercase tracking-widest text-[10px]"
            >
              Live Feed
            </Badge>
          </div>
          ...
          <Table className="min-w-[900px]">
            <TableHeader className="bg-slate-50/30">
              <TableRow className="border-slate-100 hover:bg-transparent">
                <TableHead className="text-slate-600 font-black uppercase text-[10px] tracking-[0.2em] pl-10 h-16">
                  Order ID
                </TableHead>
                <TableHead className="text-slate-600 font-black uppercase text-[10px] tracking-[0.2em]">
                  Customer
                </TableHead>
                <TableHead className="text-slate-600 font-black uppercase text-[10px] tracking-[0.2em]">
                  Date
                ...
                <TableHead className="text-slate-600 font-black uppercase text-[10px] tracking-[0.2em] text-right pr-10">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      <span
                        className="font-black text-slate-900 tracking-tight leading-none mb-1 text-base truncate max-w-[200px]"
                        title={
                          order.customer
        ...
        <DialogContent className="w-11/12 sm:min-w-fit sm:max-w-xl bg-white border-none shadow-2xl rounded-[40px] overflow-hidden p-0">
          <div className="bg-slate-900 p-10 pb-16">
            ...
            <p className="text-slate-400 font-black uppercase text-[10px] tracking-[0.2em] mt-3">
              Add a new customer order or invoice
            </p>
          </div>
          <form
          ...
          >
                ...
                <SelectContent className="rounded-2xl border-slate-100 shadow-2xl p-2">
                  {(customers || []).map((c) => (
                    <SelectItem
                      key={c.id}
                      value={c.id}
                      className="rounded-xl py-3 font-black text-slate-700"
                    >
                      ...
                      <SelectContent className="rounded-2xl border-slate-100 shadow-2xl p-2">
                        {(products || []).map((p) => (
                          <SelectItem
                            key={p.id}
                            value={p.id}
                            className="rounded-xl py-2 font-black text-slate-700"
                          >
            ...
            <div className="flex gap-4 pt-4">
              <Button
                type="button"
                variant="ghost"
                className="flex-1 h-14 text-slate-600 font-black rounded-2xl hover:bg-rose-50 hover:text-rose-600 transition-all"
                onClick={() => {
                  setShowForm(false);
                  resetForm();
                }}
