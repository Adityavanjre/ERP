...
export default function ReportsPage() {
  ...
  if (loading)
    return (
      <LoadingSpinner className="h-full" text="Loading financial reports..." />
    );
  ...
  return (
    <div className="flex-1 space-y-1.5 pt-1 md:pt-3 w-full max-w-full overflow-hidden">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Core accounting reports for compliance and period-end review.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Tabs defaultValue="trial-balance" className="space-y-1.5">
        <TabsList className="bg-slate-100 p-1.5 rounded-2xl h-auto">
          <TabsTrigger
            value="trial-balance"
            className="data-[state=active]:bg-white data-[state=active]:text-violet-600 data-[state=active]:shadow-sm rounded-xl px-3 py-2 font-bold"
          >
            Trial Balance
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="trial-balance">
          <Card className="rounded-3xl border-none shadow-xl shadow-slate-200/40">
            <CardHeader className="bg-slate-50 border-b border-slate-100 py-3 px-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                {trialBalance && (
                  <div
                    className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold ${trialBalance.balanced ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                  >
            ...
            <CardContent className="p-0 overflow-x-auto max-w-[100vw]">
              <Table className="min-w-[600px]">
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-slate-100 hover:bg-transparent">
                    <TableHead className="pl-8 text-[10px] font-black uppercase tracking-widest text-slate-500">
                      Account Name
                    </TableHead>
                    ...
                    <TableHead className="text-right pr-8 text-[10px] font-black uppercase tracking-widest text-slate-500">
                      Credit (Cr)
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {(trialBalance?.accounts || []).map(
                    (acct: TrialBalanceAccount) => (
                      <TableRow
                        key={acct.id}
                        className="border-slate-100 hover:bg-slate-50/50"
                      >
                        <TableCell
                          className="pl-8 font-bold text-slate-900 truncate max-w-[200px]"
                          title={acct.name}
                        >
                          {acct.name}
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant="secondary"
                            className="bg-slate-100 text-slate-500 text-[9px] font-black uppercase rounded-md border-none tracking-tighter"
                          >
                            {acct.type}
                          </Badge>
                        </TableCell>
              ...
              {/* Totals Row */}
              {trialBalance && (
                <div className="border-t-2 border-slate-900 py-2 px-4 flex justify-end gap-16 bg-slate-50">
                  <div className="text-right">
                    <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">
                      Total Dr
                    </div>
                    <div className="font-mono font-black text-slate-900 text-lg">
                      {fmtINR(trialBalance.totalDebit)}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">
                    ...
                    <div className="font-mono font-black text-slate-900 text-lg">
                      {fmtINR(trialBalance.totalCredit)}
                    </div>
                  </div>
                </div>
              )}
        ...
        <TabsContent value="profit-loss">
          <div className="grid lg:grid-cols-3 gap-3 mb-2">
            ...
            >
              <CardContent className="p-2.5 flex flex-col gap-2 h-full justify-center">
              ...
              <CardContent className="p-0">
                ...
                <div className="p-3 border-b border-slate-100">
                  ...
                  <div className="flex justify-between items-center pt-2 mt-2 border-t border-dashed border-slate-200">
                    ...
                    <span className="font-mono font-black text-emerald-700 text-lg">
                      {fmtINR(profitLoss?.totalRevenue || 0)}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-3">
                  ...
                  <div className="flex justify-between items-center pt-2 mt-2 border-t border-dashed border-slate-200">
                    ...
                    <span className="font-mono font-black text-red-700 text-lg">
                      {fmtINR(profitLoss?.totalExpense || 0)}
                    </span>
                  </div>
                </div>
              </CardContent>
