...
export default function AuditorDashboard() {
  ...
  const handleCloseYear = async () => {
    if (
      !confirm(
        `Are you sure you want to CLOSE the Financial Year ${year}? This will zero out all Revenue/Expense accounts and move profit to Retained Earnings.`,
      )
    )
      return;
  ...
  return (
    <div className="p-2.5 md:p-4 pb-24 md:pb-20 space-y-2 bg-slate-50 text-slate-900 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Review and close your monthly accounts.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div className="bg-white border border-slate-200 p-3 rounded-[32px] shadow-sm">
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">
            Audit Status
          </p>
          ...
          <div className="space-y-2">
            ...
            {data?.riskFlags?.map((flag: RiskFlag, i: number) => (
              ...
              >
                ...
                <span
                  className={`text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-widest ${flag.severity === "BLOCKER" ? "bg-red-600 text-white" : "bg-amber-500 text-white"}`}
                >
                  {flag.severity}
                </span>
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 p-2.5 flex flex-col sm:flex-row justify-between items-center gap-2 px-4 md:px-4 z-50">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <div
              className={`w-3 h-3 rounded-full ${data?.isLocked ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`}
            ></div>
        ...
        <div className="flex items-center gap-3">
          {data?.isLocked ? (
            ...
            <button
              ...
              className={`px-5 py-3 rounded-2xl font-black flex items-center gap-2 transition-all shadow-xl text-xs uppercase tracking-widest ${data?.status === "BLOCKED" ? "bg-slate-100 text-slate-400 cursor-not-allowed mx-auto" : "bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-500/20"}`}
            >
          ...
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-[32px] p-2.5 shadow-2xl">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-2 px-4 py-3 bg-amber-600 hover:bg-amber-500 text-white font-black rounded-2xl transition-all shadow-lg shadow-amber-600/20 text-sm uppercase tracking-widest"
              >
                Confirm Reopen
              </button>
            </div>
