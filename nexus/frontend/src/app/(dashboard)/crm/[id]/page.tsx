...
export default function CustomerDetailPage() {
  ...
  const syncRelationDetail = useCallback(
  ...
  return (
    <div className="flex-1 space-y-1.5 md:space-y-4 pt-2 md:pt-4 px-2 md:px-4 bg-slate-50 min-h-screen w-full max-w-full overflow-hidden">
      <div className="flex flex-col lg:flex-row md:items-center gap-2">
        ...
        <div>
          <h2 className="text-3xl font-black tracking-tight text-slate-900 tracking-tighter">
          ...
          <Card className="bg-white border-slate-200 shadow-xl shadow-slate-200/40 rounded-[32px] overflow-hidden border-none border-b-4 border-b-amber-500">
            <CardContent className="p-2.5 flex items-center space-x-4">
              ...
              <div>
                ...
                <div
                  className={`text-2xl font-black tracking-tighter ${currentBalance > 0 ? "text-rose-600" : "text-emerald-600"}`}
                >
                  {currencySymbol}
      ...
      <div className="grid gap-2 md:grid-cols-3">
        ...
        <Card className="bg-white border-slate-200 shadow-xl shadow-slate-200/40 rounded-[32px] md:col-span-2 border-none overflow-hidden print:col-span-3 print:border-none print:shadow-none print:bg-white print:text-black">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-2.5 flex flex-row items-center justify-between print:border-b print:border-black">
            ...
            <Button
              variant="outline"
              className="rounded-2xl border-slate-200 text-slate-600 font-bold h-10 px-5 print:hidden shadow-sm"
              onClick={() => typeof window !== "undefined" && window.print()}
            >
              <FileText className="mr-2 h-4 w-4" /> Export Record
            </Button>
          </CardHeader>
          <CardContent className="p-0 print:p-0 print:pt-3 overflow-x-auto max-w-[100vw]">
            <Table className="min-w-[700px]">
              <TableHeader className="bg-slate-50/30">
                <TableRow className="border-slate-100 hover:bg-transparent print:border-black">
                  ...
                  <TableHead className="text-right text-slate-500 font-black text-[10px] uppercase tracking-[0.2em]">
                    Debit (▲)
                  </TableHead>
                  <TableHead className="text-right text-slate-500 font-black text-[10px] uppercase tracking-[0.2em]">
                    Credit (▼)
                  </TableHead>
                  <TableHead className="text-right text-slate-500 font-black text-[10px] uppercase tracking-[0.2em] pr-4">
                    Balance
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                ...
                <TableRow className="border-slate-50 hover:bg-transparent print:border-zinc-200">
                  <TableCell className="pl-4 text-slate-400 text-[10px] font-black tracking-widest">
                    INIT
                  </TableCell>
                  <TableCell className="text-slate-400 font-bold italic text-sm">
                    Opening Balance
                  </TableCell>
                  <TableCell></TableCell>
                  <TableCell></TableCell>
                  <TableCell></TableCell>
                  <TableCell className="text-right pr-4 font-black text-slate-900">
                    {currencySymbol}0.00
                  </TableCell>
                </TableRow>
                ...
                {(ledger || []).map((entry: LedgerEntry) => (
                  <TableRow
                    ...
                    className="border-slate-50 hover:bg-slate-50/50 print:border-zinc-200 print:text-black group"
                  >
                    <TableCell className="pl-4 text-slate-900 text-xs font-black tracking-tighter print:text-black">
                      {new Date(entry.title || entry.date).toLocaleDateString()}
                    </TableCell>
                    <TableCell>
                      <span
