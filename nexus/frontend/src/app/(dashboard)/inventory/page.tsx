  ...
  const syncInventory = useCallback(
    async (showLoading = false) => {
      try {
        ...
        setStats(
          statsRes.data || { totalProducts: 0, lowStock: 0, totalValue: 0 },
        );
    ...
    <div className="flex-1 space-y-6 md:space-y-8 pt-2 md:pt-6 px-4 md:px-8 w-full max-w-full overflow-hidden">
      {fetchError && (
        <div className="bg-rose-50 border border-rose-100 p-4 rounded-2xl flex items-center gap-3 text-rose-600 font-bold text-sm mb-4">
          ...
          <Button
            variant="ghost"
            className="ml-auto text-rose-600 hover:bg-rose-100 rounded-xl"
            ...
            <form onSubmit={handleCreate} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="space-y-2 lg:col-span-2">
                  ...
                  <Input
                    className="bg-slate-50 border-slate-200 text-slate-900 rounded-xl h-11 focus:ring-blue-500/20 font-black"
                    value={formData.name}
                    onChange={(e) =>
                ...
                <div className="space-y-2 text-rose-600 bg-rose-50/30 p-2 rounded-xl border border-rose-100/50">
                  ...
                  <NumericInput
                    ...
                    onChange={(val) =>
                      setFormData({ ...formData, minStockLevel: val })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <Input
                      className="pl-9 bg-slate-50 border-slate-200 text-slate-900 rounded-xl h-11 focus:ring-blue-500/20"
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 mt-2">
                <Button
                  type="button"
                  variant="ghost"
                  className="text-slate-400 hover:text-slate-600 font-bold rounded-xl"
                  onClick={() => {
                    setShowForm(false);
                  }}
                >
                  ...
                  >
                    <div className="flex justify-between items-start relative z-10">
                      <div className="space-y-1">
                        <p className="text-[9px] text-slate-400 uppercase font-black tracking-widest">
                          SKU-{rec.sku.slice(-4).toUpperCase()}
                        </p>
                        <h4 className="text-sm font-black text-slate-900 truncate max-w-[140px]">
                          {rec.name}
                        </h4>
                      </div>
                      <Badge
                    ...
                    {rec.predictedShortage && (
                      <div className="pt-3 border-t border-slate-100 flex items-center text-[9px] text-slate-400 font-bold uppercase tracking-widest relative z-10">
                        <Clock className="h-3 w-3 mr-2 text-rose-400" /> Out of
                        stock by: {rec.predictedShortage}
                      </div>
                    )}
          ...
          <Table className="min-w-[1000px]">
            ...
            <TableBody>
              {(filteredProducts || []).map((p: Product) => (
                ...
                >
                  <TableCell className="pl-8 font-black text-[10px] text-blue-600 tracking-widest bg-slate-50/30 group-hover:bg-blue-50/30 transition-all">
                    #{p.sku.toUpperCase()}
                  </TableCell>
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        {p.tags?.split(",").map((tag: string, idx: number) => (
                          <Badge
                            key={idx}
                            variant="outline"
                            className="bg-slate-50 text-slate-400 font-bold text-[8px] rounded-md border-slate-200 uppercase tracking-tighter"
                          >
                            {tag.trim()}
                          </Badge>
                        ))}
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-2 min-w-[140px]">
                      <div className="flex justify-between text-[9px] font-black uppercase tracking-widest">
                        ...
                        <span
                          className={
                            Number(p.stock) <= Number(p.minStockLevel || 10)
                      ...
                      <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (Number(p.stock) / Math.max(50, Number(p.minStockLevel || 1) * 2)) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>
