...
export default function StockMovementsPage() {
      ...
      try {
        ...
        if (movementType === "TRANSFER") {
          if (!fromWarehouse || !toWarehouse || fromWarehouse === toWarehouse) {
            toast.error(
              "Please select different source and destination warehouses.",
            );
            return;
          }
  ...
  if (loading)
    return (
      <LoadingSpinner className="h-full" text="Loading inventory data..." />
    );
    ...
    <div className="flex-1 space-y-4 pt-1 md:pt-3 w-full max-w-full overflow-hidden">
      <div>
        ...
        <p className="text-slate-500 mt-2 font-medium">
          Log manual stock adjustments and inter-warehouse transfers.
        </p>
      </div>
        ...
        <div className="space-y-3">
          <Card className="bg-slate-900 text-white rounded-3xl border-none shadow-xl shadow-slate-900/10 overflow-hidden relative">
            ...
            <CardContent className="relative z-10 text-slate-400 font-medium">
              <p className="text-sm">
                Select a product to view its current stock availability across
                ...
                <div className="mt-6 space-y-4">
                  {warehouses.map((w) => {
                    ...
                    const stockLoc = w.stocks?.find(
                      (s: StockLocation) => s.productId === productId,
                    );
          ...
          <div className="bg-amber-50 rounded-3xl p-3 border-2 border-amber-200 flex items-start gap-4">
            ...
            <div className="text-amber-900 text-sm font-medium">
              ...
              Transfers between warehouses do not affect the general ledger.
              Manual Stock In/Out adjustments <b>automatically post</b> to the
              Inventory Adjustment account for audit compliance.
            </div>
          </div>
        </div>
      </div>
                ...
                <TableBody>
                  {filtered.map((m) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          className={`text-[9px] font-black uppercase rounded-lg border-none ${
                            m.type === "IN"
