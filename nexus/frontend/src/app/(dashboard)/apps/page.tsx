    ...
    <div className="flex-1 space-y-6 md:space-y-8 pt-2 md:pt-6 px-4 md:px-8">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-0">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Install and manage add-on modules for your business.
          </p>
        </div>
        <Button
          className="rounded-2xl bg-white border border-slate-200 text-slate-600 shadow-sm font-bold h-11 px-5"
          variant="outline"
        >
          <ShieldCheck className="mr-2 h-4 w-4 text-emerald-600" /> System
          Verified
        </Button>
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {apps.map((app) => (
          <Card
            key={app.id}
            className="bg-white border-slate-200 shadow-lg shadow-slate-200/40 rounded-3xl group overflow-hidden flex flex-col hover:shadow-2xl hover:shadow-blue-500/5 transition-all outline outline-0 hover:outline-2 hover:outline-blue-500/20"
          >
            <div
              ...
              <div className="flex gap-3 w-full">
                ...
                <Link href={`/apps/market/${app.name}`}>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-11 w-11 bg-slate-50 border border-slate-100 text-slate-400 rounded-2xl hover:bg-white hover:border-slate-300"
                  >
        ...
        {apps.length === 0 && !loading && (
          <div className="col-span-full py-32 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
            ...
            <p className="text-slate-400 mt-2 font-bold">
              No apps or modules are currently installed.
            </p>
          </div>
        )}
      </div>
