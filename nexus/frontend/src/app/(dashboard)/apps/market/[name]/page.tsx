...
export default function AppDetailPage() {
  ...
  if (loading)
    return (
      <div className="p-8 text-slate-400 font-black uppercase tracking-widest italic animate-pulse">
        Loading Details...
      </div>
    );
  if (!app)
    return (
      <div className="p-8 text-slate-900 bg-slate-50 min-h-screen font-black uppercase tracking-widest uppercase italic">
        Module [${appName}] not found in Klypso Store.
      </div>
    );
        ...
        <div className="flex-1 space-y-10">
            ...
            <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-start sm:items-center">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  <Badge
                    variant="outline"
                    className="text-blue-600 border-blue-200 bg-blue-50 font-black px-3 py-0.5 rounded-lg text-[10px]"
                  >
                    v{app.version}
                  </Badge>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
              {app.installed ? (
                <>
                  <Button
                    variant="outline"
                    className="border-rose-200 text-rose-500 hover:bg-rose-50 rounded-2xl px-6 font-bold transition-all"
                    onClick={handleUninstall}
                  >
                    <Trash2 className="mr-2 h-4 w-4" /> Remove
                  </Button>
                  <Button
                    className="bg-slate-900 hover:bg-blue-600 rounded-2xl px-10 text-white font-black shadow-xl shadow-slate-900/10 transition-all active:scale-95"
                ...
                <Button
                  className="bg-blue-600 hover:bg-blue-700 rounded-2xl px-12 text-white font-black shadow-xl shadow-blue-500/20 h-14 transition-all active:scale-95"
          ...
          <Tabs defaultValue="overview" className="w-full">
            <TabsList className="bg-slate-100/80 border border-slate-200 p-1.5 rounded-2xl h-auto flex flex-wrap max-w-full">
              <TabsTrigger
                value="overview"
                className="data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-sm rounded-xl px-10 py-3 font-bold transition-all"
              >
                Overview
              </TabsTrigger>
              <TabsTrigger
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                <div className="p-8 rounded-[32px] bg-white border border-slate-100 shadow-xl shadow-slate-200/40 space-y-3">
                  ...
                  <div className="text-3xl font-black text-slate-900 tracking-tighter">
                    0.4
                    <span className="text-sm font-bold ml-1 text-slate-300">
                      MS
                    </span>
                  </div>
                </div>
                  ...
                  {app.dependencies ? (
                    app.dependencies.split(",").map((dep: string) => (
                      ...
                      >
                        ...
                        <Badge className="bg-blue-50 text-blue-600 border-none px-3 font-black text-[9px] rounded-lg">
                          CORE_NODE
                        </Badge>
                      </div>
                    ))
            ...
            <CardContent className="space-y-6 pt-8 pb-8">
              <div className="flex items-center gap-4 group">
                ...
                <a
                  href={app.website}
                  target="_blank"
                  className="text-sm font-black text-slate-600 hover:text-blue-600 flex items-center transition-colors"
                >
            ...
            <CardContent className="pt-8 pb-8">
              <div className="flex items-center gap-5">
                <div className="flex-1 h-3 rounded-full bg-slate-100 overflow-hidden shadow-inner">
                  <div
                    className={`h-full transition-all duration-1000 ${app.installed ? "bg-gradient-to-r from-blue-500 to-indigo-600 shadow-[0_0_10px_rgba(59,130,246,0.5)]" : "bg-slate-300"}`}
                    style={{ width: app.installed ? "100%" : "0%" }}
                  />
                </div>
                <span
