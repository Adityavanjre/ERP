...
export class ConstructionService {
  ...
  async updateBOQStatus(tenantId: string, id: string, status: string) {
    ...
    if (!allowedTransitions[boq.status].includes(status)) {
      throw new BadRequestException(
        `Invalid status transition from ${boq.status} to ${status}`,
      );
    }
  ...
  async generateRABill(
    ...
    if (!boq) {
      throw new BadRequestException(
        'No Approved BOQ found for this project. RA Billing is locked.',
      );
    }
