...
describe('LogisticsService', () => {
    ...
    describe('registerVehicle', () => {
        it('should throw BadRequestException for restricted verticals', async () => {
            mockPrisma.tenant.findUnique.mockResolvedValue({ id: 't1', type: 'Retail', governanceProfile: null });

            await expect(service.registerVehicle('t1', { registrationNo: '12345' }))
        ...
        it('should throw BadRequestException for invalid registration format', async () => {
            mockPrisma.tenant.findUnique.mockResolvedValue({ id: 't1', type: 'Logistics', governanceProfile: null });

            await expect(service.registerVehicle('t1', { registrationNo: '123' }))
                .rejects.toThrow(BadRequestException);
            await expect(service.registerVehicle('t1', { registrationNo: '123' }))
        ...
        it('should register a vehicle if parameters are met', async () => {
            mockPrisma.tenant.findUnique.mockResolvedValue({ id: 't1', type: 'Logistics', governanceProfile: null });
            mockPrisma.vehicle.create.mockResolvedValue({ registrationNo: 'MH12AB1234' });

            const result = await service.registerVehicle('t1', { registrationNo: 'MH12AB1234', model: 'TruckX', type: 'Heavy', capacity: 10000 });
    ...
    describe('logFuel', () => {
        it('should calculate fuel cost accurately and trace correlation properly', async () => {
            ...
            expect(mockLedger.createJournalEntry).toHaveBeenCalledWith('t1', expect.objectContaining({
