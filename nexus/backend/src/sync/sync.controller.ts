...
export class SyncController {
  ...
  @Post('push')
  @Roles(Role.Owner, Role.Manager, Role.Biller, Role.Storekeeper, Role.Accountant)
  ...
  async pull(
    ...
    const tableList = tables ? tables.split(',') : ['products', 'suppliers', 'customers'];
    const records = await this.syncService.pull(req.user.tenantId, since, tableList);
  ...
  @Post('analytics')
  @Roles(Role.Owner, Role.Manager, Role.Biller, Role.Storekeeper, Role.Accountant)
  async analytics(@Req() req: any, @Body() body: {
    events: Array<{
      eventType: string;
      eventName: string;
