...
export class WebhookDlqProcessor extends WorkerHost {
    ...
    async process(job: Job<WebhookDlqJobData>): Promise<any> {
        const { provider, event, payload, originalTimestamp, tenantId, internalReplayUrl } = job.data;
        this.logger.warn(`[DLQ:${job.id}] Replaying failed webhook: provider=${provider}, event=${event}, attempt=${job.attemptsMade + 1}/3`);
        ...
        if (tenantId) {
            try {
                ...
                this.logger.warn(`[DLQ:${job.id}] Failed to write audit log: ${auditErr.message}`);
            }
        }

        // Replay mode: POST the original payload to an internal endpoint for reprocessing
        if (internalReplayUrl) {
            const internalSecret = this.config.get<string>('INTERNAL_WEBHOOK_REPLAY_SECRET');
            try {
                ...
                this.logger.log(`[DLQ:${job.id}] Replay succeeded: status=${response.status}`);
                return { replayed: true, status: response.status };
            } catch (err: any) {
                const status = err.response?.status;
                this.logger.error(`[DLQ:${job.id}] Replay failed: status=${status}, message=${err.message}`);
                // Throw to trigger BullMQ retry
                throw new Error(`Webhook replay failed with status ${status}: ${err.message}`);
            }
        }

        // Fallback: log to a persistent dead-letter store for manual ops intervention
        this.logger.error(`[DLQ:${job.id}] No replay URL configured. Storing in persistent dead-letter store for manual review.`);
        await (this.prisma as any).webhookDeadLetter.create({
            data: {
                provider,
                event,
                payload,
            ...
            this.logger.error(`[DLQ:${job.id}] Failed to store in dead-letter table: ${e.message}`);
        });
