...
export class BulkImportProcessor extends WorkerHost {
    ...
    async process(job: Job<BulkImportJobData>): Promise<any> {
        ...
        this.logger.log(`[JOB:${job.id}] Processing bulk import: type=${type}, tenant=${tenantId}`);
        ...
        if (existing?.status === 'Completed') {
            this.logger.warn(`[JOB:${job.id}] Duplicate import detected (${idempotencyKey}). Skipping.`);
        ...
        const jobRecord = await (this.prisma as any).backgroundJob.upsert({
            where: { idempotencyKey },
            update: { status: 'Processing', startedAt: new Date(), attempt: (existing?.attempt || 0) + 1 },
            create: {
                idempotencyKey,
                tenantId,
                userId,
        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                if (i % 10 === 0) {
                    await job.updateProgress(Math.min(95, 5 + Math.floor((i / rows.length) * 90)));
                }
            }

            await (this.prisma as any).backgroundJob.update({
                where: { id: jobRecord.id },
                data: { status: 'Completed', completedAt: new Date(), resultSummary: { processedCount } },
            });

            this.logger.log(`[JOB:${job.id}] Bulk import complete. Rows: ${processedCount}`);
            ...
            this.logger.error(`[JOB:${job.id}] Bulk import failed: ${err.message}`, err.stack);
