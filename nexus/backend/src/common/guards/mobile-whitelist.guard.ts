...
export class MobileWhitelistGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!channel) {
            throw new ForbiddenException('Security Violation: Access channel not identified.');
        }
        ...
        const isWriteAction = ['POST', 'PATCH', 'DELETE', 'PUT'].includes(request.method);
        ...
        if (isWriteAction && !mobileWriteEnabled) {
            await this.logging.log({
                ...
                channel: 'MOBILE',
                details: { method: request.method, reason: 'MOBILE_WRITE_ENABLED is set to false' },
        ...
        const actionId = this.reflector.getAllAndOverride<string>(MOBILE_ACTION_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        if (!actionId) {
            await this.logging.log({
                ...
                resource: context.getClass().name,
                channel: 'MOBILE',
                details: { handler: context.getHandler().name, reason: 'Action not in Mobile Whitelist' },
                ipAddress: request.ip,
            });
            throw new ForbiddenException(
            ...
            if (isApprovalAction) {
                ...
                if (illegalKeys.length > 0) {
                    await this.logging.log({
                        ...
                        resource: actionId,
                        channel: 'MOBILE',
                        details: { illegalKeys, reason: 'Mobile approvals must be binary-only. Mutation forbidden.' },
                        ipAddress: request.ip,
                    });
                    throw new ForbiddenException(
                ...
                if (!body.idempotencyKey) {
                    await this.logging.log({
                        ...
                        resource: actionId,
                        channel: 'MOBILE',
                        details: { reason: 'Mobile approvals/rejections require a client-generated idempotency key' },
                        ipAddress: request.ip,
                    });
                    throw new ForbiddenException(
