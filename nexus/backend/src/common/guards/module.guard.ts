...
export class ModuleGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!moduleName) {
            if (channel === 'MOBILE') {
                await this.logging.log({
                    ...
                    channel: 'MOBILE',
                    details: { handler: context.getHandler().name, reason: 'Module registration missing' },
        ...
        const isAccountingOnMobile = channel === 'MOBILE' && moduleName === 'accounting';

        if (isAccountingOnMobile || (channel === 'MOBILE' && config.mobileRestrictedModules?.includes(moduleName))) {
            const auditAction = isAccountingOnMobile ? 'MANIFESTO_VIOLATION_INV_05' : 'CHANNEL_RESTRICTION_VIOLATION';
