...
export class SystemInitService implements OnApplicationBootstrap {
  ...
  private async syncSuperAdmin() {
    ...
    if (!rawPassword) {
      throw new Error(
        'SEC-021: ADMIN_PASSWORD must be provided for Super Admin synchronization.',
      );
    }
    ...
    try {
      ...
      if (existingUser) {
        ...
        this.logger.log(
          `Super Admin verified/updated: ${adminEmail} (Nexus Primary)`,
        );
      } else {
        ...
        this.logger.log(
          `[PROVED] Super Admin created and verified: ${adminEmail}`,
        );
      }
    } catch (error) {
