...
const MAGIC_SIGNATURES: { mime: string; offset: number; signature: Buffer }[] = [
    ...
    { mime: 'image/png', offset: 0, signature: Buffer.from([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]) },
    // GIF87a or GIF89a
    { mime: 'image/gif', offset: 0, signature: Buffer.from([0x47, 0x49, 0x46, 0x38]) },
    // WebP: RIFF....WEBP
    { mime: 'image/webp', offset: 0, signature: Buffer.from([0x52, 0x49, 0x46, 0x46]) },
    // PDF: %PDF
...
export function validateFileMagicBytes(
    ...
    if (!file || !file.buffer || file.buffer.length < 8) {
        throw new UnsupportedMediaTypeException('File buffer is empty or too small to validate.');
    }
    ...
    for (const sig of MAGIC_SIGNATURES) {
        const fileSlice = buffer.slice(sig.offset, sig.offset + sig.signature.length);
