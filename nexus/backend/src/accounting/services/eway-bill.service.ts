...
export class EWayBillService {
  ...
  async generateEWayBillJson(
    ...
    if (!invoice.customer)
      throw new BadRequestException('Customer details missing for E-Way Bill');
    ...
    return {
      ...
      mainHsnCode: invoice.items[0]?.hsnCode
        ? parseInt(invoice.items[0].hsnCode)
        : 0,
      itemList: invoice.items.map((item, idx) => ({
        ...
        sgstRate: item.igstAmount.greaterThan(0)
          ? 0
          : item.gstRate.toNumber() / 2,
        cgstRate: item.igstAmount.greaterThan(0)
          ? 0
          : item.gstRate.toNumber() / 2,
      ...
      distance: isNaN(Number(transportDetails.distance))
        ? 0
        : Math.round(Number(transportDetails.distance)),
