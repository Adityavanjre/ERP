  ...
  async generateGstr1Json(
    ...
    for (const inv of invoices) {
      ...
      for (let i = 0; i < inv.items.length; i++) {
        ...
        hsnMap[hsnKey].val = this.round2(
          hsnMap[hsnKey].val + Number(item.totalAmount),
        );
      ...
      if (isB2B) {
        ...
        for (const it of itemsList) {
          ...
          b2csMap[b2csKey].txval = this.round2(
            b2csMap[b2csKey].txval + it.itm_det.txval,
          );
          b2csMap[b2csKey].iamt = this.round2(
            b2csMap[b2csKey].iamt + (it.itm_det.iamt || 0),
          );
          b2csMap[b2csKey].camt = this.round2(
            b2csMap[b2csKey].camt + (it.itm_det.camt || 0),
          );
          b2csMap[b2csKey].samt = this.round2(
            b2csMap[b2csKey].samt + (it.itm_det.samt || 0),
          );
        }
      }
    }
