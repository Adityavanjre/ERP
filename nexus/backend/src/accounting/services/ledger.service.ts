...
export class LedgerService {
  ...
  async checkPeriodLock(
    ...
    if (d > futureBound) {
      throw new BadRequestException(
        'Temporal Integrity Violation: Future-dating beyond 30 days is prohibited.',
      );
    }
    if (d < pastBound) {
      throw new BadRequestException(
        'Temporal Integrity Violation: Backdating beyond 5 years is prohibited.',
      );
    }
  ...
  async initializeTenantAccounts(
    ...
    const baseAccounts = [
      ...
      {
        name: StandardAccounts.GST_RECEIVABLE,
        type: AccountType.Asset,
        code: '1008',
      },
      ...
      {
        name: StandardAccounts.INPUT_IGST,
        type: AccountType.Asset,
        code: '1009',
      },
      {
        name: StandardAccounts.INPUT_CGST,
        type: AccountType.Asset,
        code: '1010',
      },
      {
        name: StandardAccounts.INPUT_SGST,
        type: AccountType.Asset,
        code: '1011',
      },
      {
        name: StandardAccounts.OPENING_BALANCE_EQUITY,
        type: AccountType.Equity,
        code: '3001',
      },
      {
        name: StandardAccounts.ACCOUNTS_PAYABLE,
        type: AccountType.Liability,
        code: '2001',
      },
      // OUTPUT GST accounts (GST charged to customers) remain Liability
      {
        name: StandardAccounts.OUTPUT_IGST,
        type: AccountType.Liability,
        code: '2002',
      },
      {
        name: StandardAccounts.OUTPUT_CGST,
        type: AccountType.Liability,
        code: '2003',
      },
      {
      ...
      {
        name: StandardAccounts.SALES_RETURNS,
        type: AccountType.Revenue,
        code: '4003',
      },
      { name: StandardAccounts.COGS, type: AccountType.Expense, code: '5001' },
      {
        name: StandardAccounts.PURCHASE_RETURNS,
        type: AccountType.Expense,
        code: '5005',
      },
      {
        name: StandardAccounts.SALARY_EXPENSE,
        type: AccountType.Expense,
        code: '5002',
      },
      { name: 'Retained Earnings', type: AccountType.Equity, code: '3002' },
      {
        name: StandardAccounts.FIXED_ASSETS,
        type: AccountType.Asset,
        code: '1101',
      },
      {
        name: StandardAccounts.ACCUMULATED_DEPRECIATION,
        type: AccountType.Asset,
        code: '1102',
      },
      {
        name: StandardAccounts.DEPRECIATION_EXPENSE,
        type: AccountType.Expense,
        code: '5003',
      },
      {
        name: StandardAccounts.SUPPLIER_ADVANCE,
        type: AccountType.Asset,
        code: '1012',
      },
      {
        name: StandardAccounts.CUSTOMER_ADVANCE,
        type: AccountType.Liability,
        code: '2005',
      },
    ];

    const verticalAccounts: Omit<
    ...
    if (industry === Industry.Manufacturing) {
      verticalAccounts.push(
        {
          name: StandardAccounts.RAW_MATERIAL_INVENTORY,
          type: AccountType.Asset,
          code: '1005',
        },
        {
          name: StandardAccounts.FINISHED_GOODS_INVENTORY,
          type: AccountType.Asset,
          code: '1006',
        },
        {
          name: StandardAccounts.WIP_INVENTORY,
          type: AccountType.Asset,
          code: '1007',
        },
        {
        ...
        {
          name: StandardAccounts.SCRAP_EXPENSE,
          type: AccountType.Expense,
          code: '5004',
        },
      );
      ...
      verticalAccounts.push(
        ...
        {
          name: 'Risk Provision Reserves',
          type: AccountType.Equity,
          code: '3301F',
        },
        {
          name: 'KYC & Compliance Expense',
          type: AccountType.Expense,
          code: '5301F',
        },
      );
  ...
  async createJournalEntry(
    tenantId: string,
    data: CreateJournalEntryDto & { userId?: string },
    tx?: Prisma.TransactionClient,
  ) {
    ...
    for (const t of data.transactions) {
      if (new Decimal(t.amount).lte(0)) {
        throw new BadRequestException(
          'Individual transaction amounts must be strictly greater than zero.',
        );
      }
    }
    ...
    const execute = async (client: Prisma.TransactionClient) => {
      ...
      const accountIds = [
        ...new Set(data.transactions.map((t) => t.accountId)),
      ];
      ...
      for (const t of data.transactions) {
        if (!accountMap.has(t.accountId)) {
          throw new BadRequestException(
            `Account ${t.accountId} not found or does not belong to tenant`,
          );
        }
      }
      ...
      for (const t of data.transactions) {
        ...
        const isAssetOrExpense = (
          [AccountType.Asset, AccountType.Expense] as AccountType[]
