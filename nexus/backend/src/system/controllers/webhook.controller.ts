...
export class WebhookController {
  ...
  async handleRazorpay(@Req() req: RawBodyRequest<any>, @Body() body: any) {
    ...
    if (!webhookSecret) {
      this.logger.error(
        'CRITICAL: RAZORPAY_WEBHOOK_SECRET is missing. Webhook denied.',
      );
    ...
    try {
      ...
      this.logger.error(
        'Invalid Razorpay Webhook Signature — possible spoofing attempt',
      );
    ...
    try {
      if (event && event.startsWith('subscription.')) {
        ...
        if (razorStatus === 'halted' || razorStatus === 'pending') {
          await this.billing.handleSubscriptionFailure(
            subId,
            'Payment Failed / Halted',
          );
      ...
      this.logger.error(
        `Failed to process Razorpay event '${event}': ${processingErr.message}. Dispatching to DLQ.`,
      );
      ...
      await this.dlqQueue.add(`razorpay-dlq-${event}`, dlqData, {
