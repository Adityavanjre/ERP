...
async function main() {
    ...
    const tenant = await prisma.tenant.create({
        data: {
            ...
            slug: 'woodcraft',
            type: client_1.TenantType.Manufacturing,
    ...
    const accounts = await prisma.account.createMany({
        data: [
            { tenantId: tenant.id, name: 'Cash in Hand', code: '1001', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Bank - HDFC Current', code: '1002', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Raw Material Inventory', code: '1101', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Finished Goods Inventory', code: '1102', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Work in Progress', code: '1103', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Machinery & Equipment', code: '1201', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Factory Building', code: '1202', type: client_1.AccountType.Asset },
            { tenantId: tenant.id, name: 'Accounts Payable', code: '2001', type: client_1.AccountType.Liability },
            { tenantId: tenant.id, name: 'GST Payable', code: '2101', type: client_1.AccountType.Liability },
            { tenantId: tenant.id, name: 'TDS Payable', code: '2102', type: client_1.AccountType.Liability },
            { tenantId: tenant.id, name: 'Loan - Machinery', code: '2201', type: client_1.AccountType.Liability },
            { tenantId: tenant.id, name: 'Sales - Furniture', code: '4001', type: client_1.AccountType.Revenue },
            { tenantId: tenant.id, name: 'Sales - Custom Orders', code: '4002', type: client_1.AccountType.Revenue },
            { tenantId: tenant.id, name: 'Other Income', code: '4101', type: client_1.AccountType.Revenue },
            { tenantId: tenant.id, name: 'Cost of Goods Sold', code: '5001', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Raw Material Purchase', code: '5101', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Direct Labor Cost', code: '5201', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Factory Overhead', code: '5301', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Electricity - Factory', code: '5302', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Maintenance & Repairs', code: '5303', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Salaries - Admin', code: '6001', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Rent - Office', code: '6002', type: client_1.AccountType.Expense },
            { tenantId: tenant.id, name: 'Marketing & Advertising', code: '6101', type: client_1.AccountType.Expense },
        ],
    });
    ...
    for (const rm of rawMaterials) {
        const product = await prisma.product.create({
            data: {
                ...
                name: rm.name,
                sku: rm.sku,
                price: new library_1.Decimal(rm.price),
                costPrice: new library_1.Decimal(rm.price * 0.85),
                stock: new library_1.Decimal(100),
                category: rm.category,
                hsnCode: rm.hsnCode,
                gstRate: new library_1.Decimal(rm.gstRate),
                barcode: `BAR${rm.sku}`,
            },
        });
        ...
        try {
            ...
            const stockLoc = await prisma.stockLocation.create({
                data: {
                    ...
                    quantity: new library_1.Decimal(100),
                },
            });
    ...
    for (const fp of finishedProducts) {
        const product = await prisma.product.create({
            data: {
                ...
                name: fp.name,
                sku: fp.sku,
                price: new library_1.Decimal(fp.price),
                costPrice: new library_1.Decimal(fp.price * 0.6),
                stock: new library_1.Decimal(5),
                category: fp.category,
                hsnCode: fp.hsnCode,
                gstRate: new library_1.Decimal(18),
                barcode: `BAR${fp.sku}`,
            },
        });
        ...
        try {
            ...
            const stockLoc = await prisma.stockLocation.create({
                data: {
                    ...
                    quantity: new library_1.Decimal(5),
                },
            });
    ...
    if (desk) {
        const bom = await prisma.billOfMaterial.create({
            data: {
                ...
                productId: desk.id,
                quantity: 1,
                overheadRate: new library_1.Decimal(15),
                items: {
                    create: [
                        { productId: createdRawMaterials.find(p => p.sku === 'RM-TEAK-001').id, quantity: new library_1.Decimal(4), unit: 'Planks' },
                        { productId: createdRawMaterials.find(p => p.sku === 'RM-PLY-001').id, quantity: new library_1.Decimal(1), unit: 'Sheet' },
                        { productId: createdRawMaterials.find(p => p.sku === 'RM-VAR-001').id, quantity: new library_1.Decimal(0.5), unit: 'Liters' },
                        { productId: createdRawMaterials.find(p => p.sku === 'RM-SCR-001').id, quantity: new library_1.Decimal(50), unit: 'Pieces' },
                        { productId: createdRawMaterials.find(p => p.sku === 'RM-RAIL-001').id, quantity: new library_1.Decimal(2), unit: 'Kits' },
                        { productId: createdRawMaterials.find(p => p.sku === 'RM-HAND-001').id, quantity: new library_1.Decimal(4), unit: 'Pieces' },
                    ]
                }
            }
        });
...
//# sourceMappingURL=seed-manufacturing.js.map